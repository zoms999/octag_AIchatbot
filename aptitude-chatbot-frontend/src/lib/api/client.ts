...
import { NetworkMonitor } from '../errors/networkMonitor';
...
export class ApiClient {
  ...
  async request<T = any>(
    ...
    if (!networkStatus.isOnline) {
      const error = GlobalErrorHandler.handleNetworkError();
    ...
    const operation = async (): Promise<ApiResponse<T>> => {
      try {
        ...
        // Handle error with global error handler if showErrorToast is true
        if (showErrorToast) {
